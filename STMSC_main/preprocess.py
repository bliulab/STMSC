...
def pre(adata_st_list_input, # list of spatial transcriptomics (ST) anndata objects
    ...
    if 'slice' not in adata_st.obs.columns:
        adata_st.obs['slice'] = 0

    if mslice==False:
        adata_st.obsm["graph"] = construct_spatial_graph(
